...
#define IMAGE_WIDTH 345.0f
#define IMAGE_HEIGHT 345.0f
...
- (void)viewDidLoad {
    ...
    nameString = [prefs objectForKey:@"name"];
    ...
    if ([captureManager setupSessionWithPreset:AVCaptureSessionPresetPhoto error:&error]) {
        previewView = [[UIView alloc] initWithFrame:CGRectMake(335.0f, 201.0f, 345.0f, 345.0f)];
        previewView.layer.cornerRadius = 172.5;
...
//        [previewView release];
    ...
    //------------------------------ overlay image -----------------------------------------------------//
    UIImageView *cropFrameView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"pgLq-18.jpg"]];
    cropFrameView.frame=CGRectMake(0.0, 0.0, 1024.0, 768.0);
//    cropFrameView.alpha = 0.3f;
    
    //--------------------------- capture button ----------------------------------------------------------//
    captureButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [captureButton addTarget:self action:@selector(shutterButtonPushed:)forControlEvents:UIControlEventTouchDown];
    [captureButton setTitle:@"" forState:UIControlStateNormal];
    captureButton.frame = CGRectMake(793.0, 324.0, 83.0, 86.0);
    
    //----------------------------- name label ---------------------------------------------------------------//
    nameLbl = [[UILabel alloc]initWithFrame:CGRectMake(289, 653, 450, 39)];
    nameLbl.backgroundColor = [UIColor clearColor];
    nameLbl.textColor = [UIColor whiteColor];
    nameLbl.textAlignment = NSTextAlignmentCenter;
    NSString *temp = @"Dr. ";
    nameLbl.text = [temp stringByAppendingString:nameString];
    
//    [previewView addSubview:cropFrameView];
  //-------------------- add to view --------------------------------------------------------------------------//
    [self.view addSubview:cropFrameView];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    //page9 *controller = [[page9 alloc] init];
    //controller.image = resizedImage;
    //[self.navigationController pushViewController:controller animated:YES];
    //[controller release];
